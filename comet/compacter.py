...
class MemoryCompacter:
    ...
    def compact(
        ...
        # Active skill block — surfaces "which skill was used to do
        # the work" so the compactor can tag the resulting node with
        # the working skill and Pass 2 can judge skill-layer axes
        # against the actually-loaded skill (not an inference from
        # tool-call surface alone).
        if active_skills:
            skill_list = ', '.join(active_skills)
            preceding_context += (
                f'### Active skill(s) during this buffer\n  {skill_list}\n\n'
            )
